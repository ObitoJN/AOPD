...
class NaiveRollout(BaseRollout):
    def __init__(self, module: nn.Module, config, tokenizer=None):
        ...
        Args:
            ...
            config: DictConfig
            tokenizer: the task/model tokenizer (optional)
        """
        super().__init__()
        self.config = config
        self.module = module
        ...
        if self.vocab_size is not None:
            print(f"\n[NaiveRollout] Restricting tokens to vocabulary size: {self.vocab_size}\n")
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        for _ in range(self.config.response_length):
            ...
            # Restrict tokens to vocabulary size by masking out-of-vocabulary tokens
            # This prevents sampling from tokens beyond the vocabulary size
            if self.vocab_size is not None and logits.size(-1) > self.vocab_size:
