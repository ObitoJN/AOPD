...
class VocabSizeRestrictorProcessor:
    """A logits processor that restricts tokens to vocabulary size."""

    def __init__(self, vocab_size: int):
        self.vocab_size = vocab_size

    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        """Mask out-of-vocabulary tokens by setting their logits to -inf."""
...
class HFRollout(BaseRollout):
    def __init__(self, module: nn.Module, config, tokenizer=None):
        ...
        self.config = config
        self.module = module
        self.vocab_size = tokenizer.vocab_size if tokenizer is not None else None

        if self.vocab_size is not None:
            print(f"\n[HFRollout] Restricting tokens to vocabulary size: {self.vocab_size}\n")
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        # Add logits processor to restrict tokens to vocabulary size
        logits_processors = []
        if self.vocab_size is not None:
            logits_processors.append(VocabSizeRestrictorProcessor(self.vocab_size))
