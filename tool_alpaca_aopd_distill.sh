...
# 实验名称与保存路径
EXTRA_NAME=Qwen3_4B_sft3000_CL_AOPD_ht0_beta1
...
if [ "$NODE_RANK" == "0" ]; then
    ...
    actor_rollout_ref.actor.distill_mode=aopd \
    actor_rollout_ref.actor.distill.gkd_weight=1.0 \
    actor_rollout_ref.actor.distill.opd_weight=1.0 \
    actor_rollout_ref.actor.distill.aopd_threshold=0.2 \
    actor_rollout_ref.actor.distill.jsd_beta=1 \
    ...
    trainer.project_name='tool_alpaca_aopd' \
